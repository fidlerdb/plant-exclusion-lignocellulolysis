...
df <- fread("CAZy/04_CAZyme_Origin_Analysis/Cleaned_Data/Metabolite-Correlate-CAZyme-Profiles_Phyla.csv")
...
if(!file.exists("CAZy/04_CAZyme_Origin_Analysis/heatmap_data_all_Clusters.rds")){
  ...
  saveRDS(clust_plot_object
          , file = "CAZy/04_CAZyme_Origin_Analysis/heatmap_data_all_Clusters.rds")
} else { 
  clust_plot_object <- readRDS("CAZy/04_CAZyme_Origin_Analysis/heatmap_data_all_Clusters.rds")
  }
...
metabolite <- sub("-.*", "", names(nHighProportion))
...
# Save the plot
save_plot("Figures/CAZy_Taxonomy/Heatmaps/Cluster_Overview_Heatmap_2020-09-27.pdf"
          , cp_final
          , base_height = 4, base_width = 7
          )
#plot_grid(ep, cp, align = "h", axis = "tb")
