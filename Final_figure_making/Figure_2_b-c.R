...
tf$Richness <- specnumber(df[,4:length(df)])
...
# Create a model to test if there are differences
mod2 <- glm(Richness ~ Treatment, data = tf, family = quasipoisson())
...
p_richness <- ggplot(tf, aes(x = Treatment, y = Richness
                              , colour = Treatment)) + 
  geom_point(colour = 'grey50', position = position_jitter(0.15)) +
  stat_summary(fun.data = mean_ci, size = 1.1) +
  theme_classic() + 
